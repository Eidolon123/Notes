...
pid_t pid;
    int status = 0;
    int i;
    int conpos = bufferlength+1;
    int prodpos = bufferlength+2;
  ...
  if(pid){
    ...
    while(1){ // Always run
      ...
      printf("Consumed item number %d.  Item value was %d\n",
	     consumed,item);
      consumed++;
      if(consumed==bufferlength) // Reset buffer ring when reach end
      {
        consumed=0;
      }
    }
    ...
    while(1){ // Always run
      ...
      ops_signal(id);
      produced++;
      if(produced==bufferlength) // Reset buffer ring when reach end
      {
        produced=0;
      }
    }
